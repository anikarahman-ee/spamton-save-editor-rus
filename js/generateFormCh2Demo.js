...
function generateForm(data){
  ...
  formString += "</div>";
    ...
    formString += "<div id='item_wrapper'>";
    for(var i = 0; i < 8; i++){
      formString += generateSelectCh1(`_${(i*2)+538}`,`Slot ${i+1}<br>`,data[`_${(i*2)+538}`],phone_numbers,'item_slot grey_disable');
    }
    formString += "</div>";
    formString += "</div>";
...
formString += `<div id="tab_other" class='tab'>`;
  ...
  return window.localizeGeneratedForm ? window.localizeGeneratedForm('ch2Demo', formString) : formString;
}
